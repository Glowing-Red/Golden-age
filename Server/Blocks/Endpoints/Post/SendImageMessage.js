...
ReBlock.CreateBlock(ReBlock.GenerateName(), class {
    ...
    #Route = "send-image-message:roomId/:gridFSId";
    constructor(block) {
        this.#Block = block;
    ...
    async #PostEndpoint(req, res) {
        ...
        try {
            ...
            const originalFileHash = this.GenerateHash256(req.file.buffer);
            console.log(`Mottog bild för meddelande: ${req.file.originalname}, Hash: ${originalFileHash}`);

            const imageMetadataCollection = Database.collection("ImageMetadata");
